...
const LearnArticleTab = (props: LearnArticleTabProps) => {
	...
	return (
		<>
			<div className='flex flex-col items-start justify-center w-full pt-6 mb-8'>
				<div className='flex items-center justify-between w-full'>
					<h2 className='font-black '>Catégories</h2>
					<a className='text-sm font-semibold text-blue-500' href={APP_ROUTES_ENUM.ARTICLE_CATEGORIES}>
						Voir tout &gt;
					</a>
				</div>
					...
					<div className='flex space-x-4 overflow-x-auto snap-x snap-mandatory'>
						{props.articlesCategories.length
							? props.articlesCategories.map((category) => (
								...
								>
									<a href={`${APP_ROUTES_ENUM.ARTICLE_CATEGORY}/${category.id}`}>{category.title}</a>
								</Button>
							))
